...
// GetCmdQueryDepositsByAcc implements the command to query all the deposits made from one account address.
func GetCmdQueryDepositsByAcc() *cobra.Command {
	bech32PrefixAccAddr := sdk.GetConfig().GetBech32AccountAddrPrefix()

	cmd := &cobra.Command{
		Use:   "deposits-by-acc [address]",
		Short: "Query all deposits made by one account address",
		Long: strings.TrimSpace(
			fmt.Sprintf(`Query deposits for an individual account on all houses.

Example:
$ %s query house deposits-by-acc %s1gghjut3ccd8ay0zduzj64hwre2fxs9ld75ru9p
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			params := &types.QueryDepositsByAccRequest{
				Address:    depAddr.String(),
				Pagination: pageReq,
			}

			res, err := queryClient.DepositsByAcc(cmd.Context(), params)
			if err != nil {
				return err
			}
